-- ============================================================================
-- MatelasPro - Complete Database Schema for phpMyAdmin
...
-- These indexes may already exist from table creation
-- Run these only if needed (phpMyAdmin will show error if index exists)

-- Check and create index only if it doesn't exist
SET @exist := (SELECT COUNT(*) FROM information_schema.statistics 
               WHERE table_schema = 'warehouse_db' 
               AND table_name = 'transaction' 
               AND index_name = 'idx_transaction_date_type_mattress');

SET @sqlstmt := IF(@exist = 0, 
    'CREATE INDEX idx_transaction_date_type_mattress ON transaction(date, type, mattress_id)', 
...
SET @exist := (SELECT COUNT(*) FROM information_schema.statistics 
               ...
               AND index_name = 'idx_mattress_type_quantity');

SET @sqlstmt := IF(@exist = 0, 
    'CREATE INDEX idx_mattress_type_quantity ON mattress(type, quantity)', 
    'SELECT ''Index already exists''');
...
SET @exist := (SELECT COUNT(*) FROM information_schema.statistics 
               WHERE table_schema = 'warehouse_db' 
               AND table_name = 'mattress' 
               AND index_name = 'idx_mattress_quantity_sold');

SET @sqlstmt := IF(@exist = 0, 
    'CREATE INDEX idx_mattress_quantity_sold ON mattress(quantity_sold)', 
    'SELECT ''Index already exists''');
